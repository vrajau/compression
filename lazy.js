...
function unloadImage(img) {
	...
	if(!datasrc){ // if there is no src attribute go back
		return;
	} else { // if there is a src attribute then...
...
const imgObserver = new IntersectionObserver((entries,imgObserver) => {
	entries.forEach(entry => {
		if (!entry.isIntersecting){ // if element is not intersecting then...
			unloadImage(entry.target); //fire function unload
			console.log(entry.target);


		} else{ //if element IS intersecting then...
...
imageload.forEach(image => { // for each image fire the intersection obserber and observe image
